import React from 'react'
import { render } from 'utils/tests'
import { PureHeader } from '../index'
import { siteMetadata } from '../../../../static/constants' //TODO: fix

//TODO: beforeAll - need init for i18next.language

describe('Header', () => {
  it('renders correctly', () => {
    const data = {
        site: {
          siteMetadata,
        },
        logoImage: {
          childImageSharp: {
            fluid: {
              aspectRatio: 1,
              src: '/static/4a9773549091c227cd2eb82ccd9c5e3a/09f8c/gatsby-icon.png',
              base64:
                'data:image/png;base64,',
              srcSet:
                '/static/4a9773549091c227cd2eb82ccd9c5e3a/2db51/gatsby-icon.png 13w,\n/static/4a9773549091c227cd2eb82ccd9c5e3a/8505e/gatsby-icon.png 25w,\n/static/4a9773549091c227cd2eb82ccd9c5e3a/09f8c/gatsby-icon.png 50w,\n/static/4a9773549091c227cd2eb82ccd9c5e3a/e22c9/gatsby-icon.png 75w,\n/static/4a9773549091c227cd2eb82ccd9c5e3a/bf65b/gatsby-icon.png 100w,\n/static/4a9773549091c227cd2eb82ccd9c5e3a/774d6/gatsby-icon.png 512w',
              sizes: '(max-width: 50px) 100vw, 50px',
            },
          },
        },
      },
      header = render(<PureHeader data={data} />)

    expect(header.getByTestId('header-logo-title')).toBeInTheDocument()

    // getByTestId('header-logo-image') don't working
    const headerLogoImage = header.container.querySelector('[data-testid=header-logo] picture')
    expect(headerLogoImage).toBeInTheDocument()

    expect(header.asFragment()).toMatchSnapshot()
  })
})
